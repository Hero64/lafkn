...
describe('Queue send message integration', () => {
  ...
  it('should create queue integration with global resource', async () => {
    ...
    const queue = alicantoResource.create('queue', SqsQueue, stack, 'test', {
      name: 'test',
    });
    queue.isGlobal();
    ...
    expect(synthesized).toHaveResourceWithProperties(ApiGatewayIntegration, {
      ...
      type: 'AWS',
      uri: 'arn:aws:apigateway:${aws_api_gateway_rest_api.testing-rest-api.region}:sqs:path/${aws_sqs_queue.test.id}',
    });
  });
  ...
  it('should create queue integration with event props', async () => {
    ...
    expect(synthesized).toHaveResourceWithProperties(ApiGatewayIntegration, {
      ...
      type: 'AWS',
      uri: 'arn:aws:apigateway:${aws_api_gateway_rest_api.testing-rest-api.region}:sqs:path/test',
    });
  });
});
