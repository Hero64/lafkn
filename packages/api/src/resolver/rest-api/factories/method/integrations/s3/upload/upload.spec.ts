...
describe('Bucket upload integration', () => {
  ...
  class BucketIntegrationApi {
    ...
    @Get({
      path: 'upload/event/{bucket}',
      action: 'Upload',
      integration: 'bucket',
    })
    uploadEvent(@Event(Upload) e: Upload): BucketIntegrationResponse {
      return {
        bucket: e.bucket,
        object: e.object,
      };
  ...
  it('should create s3 integration', async () => {
    ...
    const handler = handlers.find((h) => h.name === 'upload') as ApiLambdaMetadata;
  ...
  it('should create s3 integration with event prop', async () => {
    ...
    const handler = handlers.find((h) => h.name === 'uploadEvent') as ApiLambdaMetadata;
